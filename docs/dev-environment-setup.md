...
### 2.6 Port Forward to Expose ADB on All Interfaces

The emulator's ADB only listens on `127.0.0.1:5555` (localhost). To make it reachable from the Linux server over Tailscale, add a port proxy rule (Administrator PowerShell):
...
This rule persists across reboots — you only need to run it once.

### 2.7 Find Your Windows Tailscale IP
