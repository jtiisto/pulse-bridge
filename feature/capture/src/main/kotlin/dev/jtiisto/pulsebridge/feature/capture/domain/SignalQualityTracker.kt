...
import dev.jtiisto.pulsebridge.feature.capture.domain.model.SignalQuality
import dev.jtiisto.pulsebridge.feature.capture.domain.model.SignalQualityLevel
import kotlin.math.abs
import kotlin.math.roundToInt

/**
 * Rolling live estimate of RR signal quality for DFA α1, from the same beat
 * stream that feeds the tachogram. Not a full analysis — a fast, honest read
 * using the offline module's gates (≥95% RR coverage, ≤5% artifacts, no gap).
 *
 ...
 * veto) MUST match the offline analysis in server/analysis/quality.py and
 * server/analysis/pipeline.py `_window_alpha1`. "GOOD" here promises the
 * server will trust the window — if the two drift, that promise breaks. There
 * is no shared test enforcing this cross-language parity; change one side and
 * you MUST change the other. See specs/signal_quality_indicator.md.
 *
 * One deliberate, documented approximation: the ectopic median is trailing
 * here vs centered offline (a live stream has no future beats) — see
 * MEDIAN_WINDOW. It only affects near-threshold beats mid-transition.
 *
