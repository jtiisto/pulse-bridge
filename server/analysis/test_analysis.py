...
from analysis.hr import hr_summary, _durations_s
from analysis.segment import _smooth, detect_bouts
from analysis.pipeline import analyze
from analysis import report as report_mod
from analysis.db import _normalize_ts
...
def test_polar_timestamps_normalized_to_end_anchor():
    # Polar start-anchored ts shifts forward by its RR; Garmin unchanged
    assert _normalize_ts(1000, 800, "polar_pvs") == 1800
    assert _normalize_ts(1000, 800, "garmin_hrm") == 1000
    assert _normalize_ts(1000, 0, "polar_pvs") == 1000  # zero-PPI unchanged
