...
from analysis.quality import Beat


def _square_wave(rounds=4, work_hr=160, rest_hr=100, seconds=60, t0=1_700_000_000_000):
...
def _steady(beats_count=600, rr=700, hr=86, t0=1_700_000_000_000):
...
def test_write_png_handles_windows_with_no_alpha1(tmp_path):
    ...
    beats = [Beat(ts_ms=1_700_000_000_000 + i * 700, rr_ms=700, hr_bpm=86,
                  is_gap=(i % 3 == 0))
             for i in range(600)]
    report = analyze(beats)
...
def test_write_png_with_no_usable_hr_samples(tmp_path):
    ...
    beats = [Beat(ts_ms=1_700_000_000_000 + i * 700, rr_ms=700, hr_bpm=0, is_gap=False)
             for i in range(400)]
    report = analyze(beats)
